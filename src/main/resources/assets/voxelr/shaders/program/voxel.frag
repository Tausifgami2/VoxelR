...
void main() {
    ...
    // Cap steps for rays hitting y-bound
    if (rayDir.y > 0.0) {
    ...
    // Water surface tracking (w < 0 = no hit yet)
    vec4 waterCol = vec4(0.0, 0.0, 0.0, -1.0);
    ...
    for (uint i = 0u; i < maxSteps; i++) {
        ...
        if (bi != 0u && shapeOffsets[bi] == -1) {
            ...
            if (tt == 3u) {
                // Water surface — store and continue
                if (waterCol.w < 0.0) {
                    ...
                    vec4 uvRect = atlasUVs[bi * 6u + atlasFace];
                        ...
                        waterCol = vec4(col, max(texColor.a, 0.3));
                ...
                if (texColor.a >= 0.5) {
                    ...
                    col *= clamp(u_AmbientLight, 0.1, 1.0);
                    if (waterCol.w >= 0.0) { col = mix(col, waterCol.xyz, waterCol.w); }
